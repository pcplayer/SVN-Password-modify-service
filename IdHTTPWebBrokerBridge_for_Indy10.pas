...
var
  s: string;
begin
  if (Index = INDEX_ContentLength) or (Index = INDEX_Content) then
  begin
    if Assigned(FRequestInfo.PostStream) then
    begin
      // has an explicit content stream...
      FRequestInfo.PostStream.Position := 0;
      s := ReadStringAsCharSet(FRequestInfo.PostStream, FRequestInfo.CharSet);
      // TODO: for INDEX_ContentLength, use the FRequestInfo.ContentLength
      // or PostStream.Size property instead?
    end
    else if FRequestInfo.FormParams <> '' then begin {do not localize}
      // an input form that was submitted as "application/www-url-encoded"...
      s := FRequestInfo.FormParams;
    end else
    begin
      // anything else for now...
      s := FRequestInfo.UnparsedParams;
    end;
  end;
  case Index of
    ...
    INDEX_Query           : Result := AnsiString(FRequestInfo.UnparsedParams);
    ...
    INDEX_ContentLength   : Result := AnsiString(IntToStr(Length(s)));
    ...
    INDEX_Content         : Result := AnsiString(s);
...
begin
  ...
  if LLength > 0 then
    begin
    Result := AnsiString(copy(FRequestInfo.UnparsedParams, FClientCursor, LLength));
